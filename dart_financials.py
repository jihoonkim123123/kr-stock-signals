...
def main():
    ...
    # ─── GitHub Issue 본문용 마크다운 (workflow가 직접 사용) ───
    full_s = trade_stats(all_trades, port_full["equity_pts"])
    in_s = trade_stats(in_trades, port_in["equity_pts"])
    out_s = trade_stats(out_trades, port_out["equity_pts"])
    pf_s = lambda s: "inf" if not np.isfinite(s.get("profit_factor", 0)) else f"{s['profit_factor']:.2f}"
    md_lines = [
        f"## 🧪 백테스트 결과 — {dt.date.today()}",
        "",
        f"**기간**: {full_start} ~ {full_end} ({CONFIG['test_years']}년) · "
        f"**유니버스**: {CONFIG['universe']} · "
        f"**가중치**: 기술 {CONFIG['w_technical']*100:.0f}% + 수급 {CONFIG['w_supply']*100:.0f}% + 모멘텀 {CONFIG['w_momentum']*100:.0f}%",
        "",
        "### 📊 전체 기간 (Full Sample)",
        f"- 거래수 **{full_s.get('n',0)}** · 승률 **{full_s.get('win_rate',0):.1f}%** · "
        f"기대값/거래 **{full_s.get('expectancy',0):+.2f}%** · PF **{pf_s(full_s)}**",
        f"- 자산: ₩{CONFIG['initial_capital']:,.0f} → **₩{port_full['final']:,.0f}** "
        f"(**{port_full['total_return_pct']:+.1f}%**)",
        f"- MDD **{full_s.get('mdd',0):.1f}%** · Sharpe **{full_s.get('sharpe',0):.2f}** · "
        f"평균보유 {full_s.get('avg_days',0):.1f}일",
        "",
        f"### 🔬 In-Sample ({full_start} ~ {cutoff})",
        f"- 거래수 {in_s.get('n',0)} · 승률 {in_s.get('win_rate',0):.1f}% · PF {pf_s(in_s)} · "
        f"MDD {in_s.get('mdd',0):.1f}% · Sharpe {in_s.get('sharpe',0):.2f}",
        f"- 누적 **{port_in['total_return_pct']:+.1f}%** "
        f"(연환산 {annualize_return(port_in['total_return_pct'], in_years):+.1f}%)",
        "",
        f"### 🎯 Out-of-Sample ({cutoff} ~ {full_end}) — **진짜 검증**",
        f"- 거래수 {out_s.get('n',0)} · 승률 {out_s.get('win_rate',0):.1f}% · PF {pf_s(out_s)} · "
        f"MDD **{out_s.get('mdd',0):.1f}%** · Sharpe **{out_s.get('sharpe',0):.2f}**",
        f"- 누적 **{port_out['total_return_pct']:+.1f}%** "
        f"(연환산 **{annualize_return(port_out['total_return_pct'], out_years):+.1f}%**)",
        "",
        f"### 📈 vs KOSPI 매수보유",
        f"- KOSPI {CONFIG['test_years']}년: {bench['return_pct']:+.1f}% "
        f"(연환산 {annualize_return(bench['return_pct'], CONFIG['test_years']):+.1f}%)",
    ]
    if mc:
        md_lines += [
            "",
            f"### 🎲 Monte Carlo ({mc['n_iter']}회 재샘플)",
            f"- 누적: 중앙값 **{mc['return_median']:+.1f}%** · "
            f"5%-95% [{mc['return_p5']:+.1f}%, {mc['return_p95']:+.1f}%]",
            f"- MDD: 평균 {mc['mdd_mean']:.1f}% · 95%-percentile {mc['mdd_p95']:.1f}% · 최악 **{mc['mdd_worst']:.1f}%**",
            f"- 수익 확률: **{mc['prob_profit']:.1f}%**",
        ]
    oos_sharpe = out_s.get("sharpe", 0)
    oos_mdd = out_s.get("mdd", 0)
    oos_ret = port_out["total_return_pct"]
    bench_oos = bench["return_pct"] * (out_years / max(CONFIG["test_years"], 1))
    md_lines += ["", "### 🤖 자동 평가"]
    md_lines.append(("- ✅" if oos_sharpe >= 0.8 else "- ⚠️") +
                    f" OOS Sharpe **{oos_sharpe:.2f}** " +
                    ("≥ 0.8 — 양호" if oos_sharpe >= 0.8 else "< 0.8 — 변동성 대비 수익 부족"))
    md_lines.append(("- ✅" if oos_mdd > -25 else "- ⚠️") +
                    f" OOS MDD **{oos_mdd:.1f}%** " +
                    ("— 운용 가능" if oos_mdd > -25 else "— 너무 깊음"))
    md_lines.append(("- ✅" if oos_ret > bench_oos else "- ⚠️") +
                    f" OOS 누적 **{oos_ret:+.1f}%** vs KOSPI {bench_oos:+.1f}% " +
                    ("— 알파 있음" if oos_ret > bench_oos else "— 인덱스 대비 약함"))
    md_lines += [
        "",
        "---",
        f"⚠️ 거래비용·슬리피지·거래세 반영 · 유동성 필터 ₩{CONFIG['min_daily_value']:,.0f}/일 이상 · "
        f"백테스트는 미래 수익을 보장하지 않습니다.",
    ]
    md_path = OUTPUT_DIR / "backtest_summary.md"
    md_path.write_text("\n".join(md_lines), encoding="utf-8")
    print(f"📝 이슈용 마크다운 → {md_path}")
...
__ORPHAN_END__ = """
        ...
        f"- 자산: ₩{CONFIG['initial_capital']:,.0f} → **₩{port_full['final']:,.0f}** "
        f"(**{port_full['total_return_pct']:+.1f}%**)",
        f"- MDD **{full_s.get('mdd',0):.1f}%** · Sharpe **{full_s.get('sharpe',0):.2f}** · 평균보유 {full_s.get('avg_days',0):.1f}일",
        "",
        f"### 🔬 In-Sample ({full_start} ~ {cutoff})",
        f"- 거래수 {in_s.get('n',0)} · 승률 {in_s.get('win_rate',0):.1f}% · PF {pf_s(in_s)} · MDD {in_s.get('mdd',0):.1f}% · Sharpe {in_s.get('sharpe',0):.2f}",
        f"- 누적 **{port_in['total_return_pct']:+.1f}%** (연환산 {annualize_return(port_in['total_return_pct'], in_years):+.1f}%)",
        "",
        f"### 🎯 Out-of-Sample ({cutoff} ~ {full_end}) — 진짜 검증",
        f"- 거래수 {out_s.get('n',0)} · 승률 {out_s.get('win_rate',0):.1f}% · PF {pf_s(out_s)} · MDD **{out_s.get('mdd',0):.1f}%** · Sharpe **{out_s.get('sharpe',0):.2f}**",
        f"- 누적 **{port_out['total_return_pct']:+.1f}%** (연환산 **{annualize_return(port_out['total_return_pct'], out_years):+.1f}%**)",
        "",
        f"### 📈 KOSPI 매수보유 {CONFIG['test_years']}년: {bench['return_pct']:+.1f}% (연환산 {annualize_return(bench['return_pct'], CONFIG['test_years']):+.1f}%)",
    ]
    if mc:
        md_lines += [
            "",
            f"### 🎲 Monte Carlo ({mc['n_iter']}회)",
            f"- 누적: 중앙값 **{mc['return_median']:+.1f}%** · 5%-95% [{mc['return_p5']:+.1f}%, {mc['return_p95']:+.1f}%]",
            f"- MDD: 평균 {mc['mdd_mean']:.1f}% · 95p {mc['mdd_p95']:.1f}% · 최악 **{mc['mdd_worst']:.1f}%**",
            f"- 수익 확률: **{mc['prob_profit']:.1f}%**",
        ]
    oos_sharpe = out_s.get("sharpe", 0)
    oos_mdd = out_s.get("mdd", 0)
    ...
    bench_oos = bench["return_pct"] * (out_years / max(CONFIG["test_years"], 1))
    md_lines += ["", "### 🤖 자동 평가"]
    md_lines.append(("- ✅" if oos_sharpe >= 0.8 else "- ⚠️") + f" OOS Sharpe **{oos_sharpe:.2f}**")
    md_lines.append(("- ✅" if oos_mdd > -25 else "- ⚠️") + f" OOS MDD **{oos_mdd:.1f}%**")
    md_lines.append(("- ✅" if oos_ret > bench_oos else "- ⚠️") + f" OOS 누적 **{oos_ret:+.1f}%** vs KOSPI {bench_oos:+.1f}%")
    md_lines += ["", "---", f"⚠️ 거래비용·슬리피지·거래세 반영 · 유동성 필터 ₩{CONFIG['min_daily_value']:,.0f}/일 이상."]
    md_path = OUTPUT_DIR / "backtest_summary.md"
    md_path.write_text("\n".join(md_lines), encoding="utf-8")
    print(f"📝 이슈용 마크다운 → {md_path}")
